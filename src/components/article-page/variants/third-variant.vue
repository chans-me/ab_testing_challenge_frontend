<template>
  <div>
    <v-card class="pa-lg-8 pa-md-6">
      <v-card-title class="display-2 green--text text--darken-4" style="word-break: none !important;">
        <p>Making Sound Waves: How Blinkist Creates Award-Winning Audio</p>
      </v-card-title>
      <v-card-subtitle class="display-1 green--text text--darken-4">
        How did we turn Blinkist from a text-only app into an audio powerhouse? Let's take a look at the decks to find out.
      </v-card-subtitle>
      <v-card-text class="green--text text--darken-4">
      ...
      <v-card-actions class="justify-center">
        <v-btn color="green darken-4 white--text" x-large @click="$emit('signup')">Start Your Free Trial</v-btn>
      </v-card-actions>
    </v-card>
  </div>
</template>
